...
#region Culling
        ...
        void UpdateCulling()
        {
            ...
            {
                bool visible = true;
                if (cullingEnabled && m_Master.beamGeometry)
                {
                    if (mainCamera)
                    {
                        var renderer = m_Master.beamGeometry.meshRenderer;
                        var maxDistSqr = cullingMaxDistance * cullingMaxDistance;
                        var distSqr = renderer.bounds.SqrDistance(mainCamera.transform.position);
